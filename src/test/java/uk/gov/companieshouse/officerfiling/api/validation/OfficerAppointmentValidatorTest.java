...
//        officerAppointmentValidator = new OfficerAppointmentValidator(logger, companyProfileService, postcodeValidationService, apiEnumerations, allowedNationalities,List.of ("England", "Wales", "Scotland", "Northern Ireland", "France"), List.of ("England", "Wales", "Scotland", "Northern Ireland"));
...
//        when(postcodeValidationService.validUKPostcode(dto.getServiceAddress().getPostalCode())).thenReturn(true);
...
//                .build());
////        when(postcodeValidationService.validatePostcodeExistence(dto.getResidentialAddress().getPostalCode())).thenReturn(false);
//        when(apiEnumerations.getValidation(ValidationEnum.PREMISES_LENGTH)).thenReturn(
//                "Property name or number must be 200 characters or less");
//        when(apiEnumerations.getValidation(ValidationEnum.ADDRESS_LINE_ONE_LENGTH)).thenReturn(
//                "Address line 1 must be 50 characters or less");
...
//        when(apiEnumerations.getValidation(ValidationEnum.POSTAL_CODE_LENGTH)).thenReturn(
//                "Postal code must be 20 characters or less");
//        when(apiEnumerations.getValidation(ValidationEnum.POST_CODE_INVALID)).thenReturn(
...
//                .contains("County, state, province or region must be 50 characters or less")
//                .contains("Postal code must be 20 characters or less")
//                .contains("Enter a real postcode");
//    }
//
//    @Test
...
//
//    @Test
//    void validationWhenPostcodeValidationServiceReturnsFalse() {
//        setupDefaultParamaters();
//        when(dto.getServiceAddress()).thenReturn(AddressDto.builder(validCorrespondenceAddressInUK).build());
//        when(postcodeValidationService.validUKPostcode(dto.getServiceAddress().getPostalCode())).thenReturn(false);
//
//        when(apiEnumerations.getValidation(ValidationEnum.POST_CODE_INVALID)).thenReturn(
//                "Enter a real postcode");
//        final var apiErrors = officerAppointmentValidator.validate(request, dto, transaction,
//                PASSTHROUGH_HEADER);
//        assertThat(apiErrors.getErrors())
//                .as("Errors when postcode service returns false")
//                .hasSize(1)
//                .extracting(ApiError::getError)
//                .contains("Enter a real postcode");
//    }
